...
/**
 基础设置
 */
@property(nonatomic, strong) WaveConfig *config;
...
#pragma mark - 快速创建方法
+ (instancetype)waveViewWithConfig:(void (^)(WaveConfig *))config
{
    WaveView *waveView = [[WaveView alloc] initWithConfig:config];
    return waveView;
}
- (instancetype)initWithConfig:(void (^)(WaveConfig *))config
{
    WaveView *waveView = [[WaveView alloc] init];
    if (config) {
...
- (void)wave
{
    ...
    switch (self.config.position) {
        case WavePositionTop: // 顶部绘制
            CGPathAddLineToPoint(path, nil, self.width, 0);
            CGPathAddLineToPoint(path, nil, 0, 0);
            break;
        case WavePositionBottom:  //  底部绘制
...
{
    if (_config == nil) {
        _config = [WaveConfig defaultConfig];
    }
    return _config;
}
- (UIImageView *)imgView
